...
public:
    ...
    void Run(void* ptr)
    {
      JobData* pJD = (JobData*) ptr;
		CTreeNode* pNode = pJD->pNode;
		TCondition& nodesCond = *pJD->pNodesCond;

		bool notLeaf = pNode->split(pJD->alpha);
		
		nodesCond.Lock();
		if(notLeaf)
		{
			pJD->pNodes->push(pNode->left);
			pJD->pNodes->push(pNode->right);
			*pJD->pToDoN += 1;
		}
		else
			*pJD->pToDoN -= 1;
...
{	
	...
	while(!nodes.empty())
	{
		CTreeNode* pNode = nodes.top(); //pointer to the node that will be attempted to split next
		nodes.pop();

		bool notLeaf = pNode->split(alpha);
	...
	while(toDoN > 0)
	{
		...
		if(toDoN > 0)
		{
			...
			JobData* pJD = new JobData(pNode, &nodes, &nodesCond, &toDoN, alpha);
